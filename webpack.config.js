...
var config = {
    ...
    module: {
        rules: [
            ...
            // {     test: /\.(png|jpg|gif|svg)$/,     use: [         {             loader:
            // 'url-loader',             options: {                 limit: 8192,     name:
            // '[name].[hash:7].[ext]'             }         },         {    loader:
            // 'img-loader',             options: {                 enabled: NODE_ENV !==
            // 'development'             }         }     ] },
            {
                test: /\.css$/,
...
if (NODE_ENV !== "production") {
    ...
    if (process.env.FROM_NODE) {
        ...
        config.plugins.push(
            new OpenBrowserPlugin({ url: `http://localhost:${_PORT}` })
        );
